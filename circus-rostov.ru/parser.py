...
class CircusRostovParser:
    """
        Парсер для площадки https://www.circus-rostov.ru/.

        """
    PARSER_NAME = "circus-rostov.ru"
    EVENTS_HOOK_HELP = 'Пример, https://www.circus-rostov.ru/'
    TICKETS_HOOK_HELP = "Пример, https://widget2.kassy.ru/auth/circus.ru/?back=/rnd/event/705/"
    ...
    def get_event_data(self, hook):
        ...
        for event in events.xpath('//div[@class="ticket_item text-center"]'):
            day = event.xpath('p')[0].text
            month = RussianLocale.month_names.index(event.xpath('p')[1].text.lower())
            ...
            for circus_sessions in event.xpath('a'):
                ...
                if 'Продано' in hour:
                    continue
                title = 'Новогодняя сказка в цирке'
                event_id = str(circus_sessions.xpath('@data-kassy-event_id')[0])
                event_hook = 'https://widget2.kassy.ru/auth/circus.ru/?back=/rnd/event/{0}/'.format(event_id)
    ...
    def get_tickets_data(self, seance_hook):
        ...
        res = session.get(seance_hook)
        cookies = session.cookies.get_dict()
        event_id = seance_hook.split('/')[8]
        session_id = res.url.split('session_id=')[1]
        result = self._kassy_get_tickets_data(event_id, session_id, cookies)
        result_dict['tickets'] = result
        return result_dict

    def _kassy_get_tickets_data(self, event_id, session_id, cookies):
        headers = {
        ...
        tickets = []
        content = requests.get('https://widget2.kassy.ru/api/hall/', headers=headers, params=params,
                                   cookies=cookies)
