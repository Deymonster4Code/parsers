...
class CircusSamaraParser:
    """
    Парсер для площадки https://www.circus-samara.ru/.

    """
    PARSER_NAME = "circus-samara.ru"
    EVENTS_HOOK_HELP = 'Пример, https://www.circus-samara.ru/'
    TICKETS_HOOK_HELP = "Пример, https://widget2.kassy.ru/auth/circus.ru/?back=/samara/event/520/"
    ...
    def get_event_data(self, hook):
        ...
        for event in events.xpath('//div[@class="row"]/div/div/div/div/div/div[@class="ticket_item text-center"]'):
            ...
            for circus_sessions in event.xpath('a'):
                ...
                if 'Продано' in hour:
                    continue
                title = 'Цирк Самара ({0}.{1}.{2} {3})'.format(day, month, year, hour.split(' ')[2])
